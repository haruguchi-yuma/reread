...
RSpec.describe Photo, type: :model do
  ...
  it '5MB以上の画像であれば無効な状態であること' do
    photo = build(:photo, :with_small_jpeg)
    photo.valid?
    expect(photo.errors[:image]).not_to include('サイズは5.0MB以内で選択してください')

    over_size_photo = build(:photo, :with_large_jpeg)
  ...
  it 'メモが入力されていなくても有効なこと' do
    photo = build(:photo, note: nil)
    expect(photo).to be_valid
  end

  it 'メモに141文字以上入力されている場合は無効な状態であること' do
    photo = build(:photo, note: 'a' * 140)
    expect(photo).to be_valid

    long_note_photo = build(:photo, note: 'a' * 141)
