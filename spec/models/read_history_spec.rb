...
RSpec.describe ReadHistory, type: :model do
  it '有効なファクトリがあること' do
    expect(build(:read_history)).to be_valid
  end

  it 'サマリーがなければ無効な状態であること' do
    read_history = build(:read_history, summary: nil)
    read_history.valid?
    expect(read_history.errors[:summary]).to include('を入力してください')
  end

  it 'サマリーが51文字以上であれば無効な状態であること' do
    read_history = build(:read_history, summary: 'a' * 50)
    read_history.valid?
    expect(read_history.errors[:summary]).not_to include('は50文字以内で入力してください')

    long_summary_read_history = build(:read_history, summary: 'a' * 51)
  ...
  it 'メモが301文字以上であれば無効な状態であること' do
    read_history = build(:read_history, description: 'a' * 300)
    read_history.valid?
    expect(read_history.errors[:description]).not_to include('は300文字以内で入力してください')

    long_description_read_history = build(:read_history, description: 'a' * 301)
